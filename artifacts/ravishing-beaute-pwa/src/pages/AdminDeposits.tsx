...
const DEFAULT_SETTINGS: Settings = { depositAmount: "25" };
...
function appleCashMessage(booking: Booking, settings: Settings) {
  ...
  return `Hi ${booking.clientName}, this is Shawna with Ravishing Beauté. Your ${amount} deposit is needed to secure your ${booking.serviceLabel} appointment request for ${dateLabel(booking)}. You can tap the ${amount} amount in this iMessage and send it with Apple Cash. Please reply once sent.`;
}
    ...
    return (
      <div style={{ backgroundColor: "#F9F5F0", minHeight: "100vh", padding: 24, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: "100%", maxWidth: 380, backgroundColor: "#fff", border: "1px solid #E4D3D8", borderRadius: 18, padding: 20 }}>
          ...
          <input type="password" value={password} onChange={(event) => { setPassword(event.target.value); setPasswordError(""); }} onKeyDown={(event) => event.key === "Enter" && login()} placeholder="Admin password" style={{ width: "100%", boxSizing: "border-box", padding: "14px", border: `1.5px solid ${passwordError ? "#E04040" : "#E4D3D8"}`, borderRadius: 12, fontSize: 15, outline: "none", marginBottom: 10 }} />
    ...
    <div style={{ backgroundColor: "#F9F5F0", minHeight: "100vh", paddingBottom: 40 }}>
      ...
      <div style={{ padding: "14px 16px 0" }}>
        <div style={{ backgroundColor: "#fff", border: "1px solid #E4D3D8", borderRadius: 16, padding: 14, marginBottom: 12 }}>
          ...
          <p style={{ fontSize: 12.5, color: "#6E565C", lineHeight: 1.45 }}>Tap Apple Cash Text to open iMessage with a {amount} deposit message. The client can tap the {amount} amount in iMessage and send through Apple Cash.</p>
          {notice && <p style={{ fontSize: 12, color: "#7D6268", marginTop: 8 }}>{notice}</p>}
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
          {[["due", `Due ${counts.due}`], ["paid", `Paid ${counts.paid}`], ["all", `All ${counts.all}`]].map(([value, label]) => {
            const selected = filter === value;
            return <button key={value} onClick={() => setFilter(value as DepositFilter)} style={{ border: `1px solid ${selected ? "#AC5D7A" : "#E4D3D8"}`, backgroundColor: selected ? "#AC5D7A" : "#fff", color: selected ? "#fff" : "#7D6268", borderRadius: 999, padding: "10px 8px", fontSize: 12, fontWeight: 800 }}>{label}</button>;
          })}
        </div>

        {loading ? <p style={{ textAlign: "center", color: "#7D6268", paddingTop: 30 }}>Loading deposits…</p> : shownBookings.length === 0 ? <p style={{ textAlign: "center", color: "#7D6268", paddingTop: 30 }}>No deposit records found.</p> : shownBookings.map((booking) => {
          const phoneDigits = digits(booking.phone);
          const smsHref = `sms:${phoneDigits || booking.phone}?body=${encodeURIComponent(appleCashMessage(booking, settings))}`;
          return (
            <div key={booking.id} style={{ backgroundColor: "#fff", border: "1px solid #E4D3D8", borderRadius: 16, padding: 14, marginBottom: 10, boxShadow: "0 10px 24px rgba(82,42,57,0.045)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
                <div><p style={{ fontSize: 16, fontWeight: 800, color: "#201B1C", marginBottom: 2 }}>{booking.clientName}</p><p style={{ fontSize: 12, color: "#7D6268" }}>{booking.phone}</p></div>
              ...
              <p style={{ fontSize: 12, color: "#7D6268", lineHeight: 1.45, marginBottom: 10 }}>{dateLabel(booking)} • {booking.timePreference || "flexible"} • {amount} deposit</p>
