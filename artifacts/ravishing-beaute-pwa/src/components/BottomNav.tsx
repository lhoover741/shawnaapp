...
const tabs = [
  ...
  { path: "/notifications", icon: BellIcon, label: "App" },
  { path: "/gallery", icon: GalleryIcon, label: "Gallery" },
];

function HomeIcon({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? "#AC5D7A" : "#9CA3AF"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function BottomNav() {
  ...
  function handleBookTap() {
    const now = Date.now();
    if (!bookTapStartedAt.current || now - bookTapStartedAt.current > ADMIN_TAP_WINDOW_MS) {
      bookTapStartedAt.current = now;
      bookTapCount.current = 0;
    }
    bookTapCount.current += 1;
    if (bookTapResetTimer.current) clearTimeout(bookTapResetTimer.current);
    bookTapResetTimer.current = setTimeout(resetBookTapTrigger, ADMIN_TAP_WINDOW_MS);
  ...
  return (
    <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, backgroundColor: "#fff", borderTop: "1px solid #E4D3D8", display: "flex", paddingBottom: "env(safe-area-inset-bottom, 0px)", zIndex: 50 }}>
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, paddingTop: 10, paddingBottom: 10, background: "none", border: "none", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}
          >
            <Icon active={active} />
            <span style={{ fontSize: 10, fontWeight: active ? 600 : 400, color: active ? "#AC5D7A" : "#9CA3AF", lineHeight: 1 }}>
              {tab.label}
            </span>
          </button>
        );
