...
const SERVICES: Record<string, string> = {
  ...
  // Legacy IDs retained so existing saved/admin links still resolve gracefully.
...
router.post("/booking-requests", async (req: Request, res: Response) => {
  try {
    ...
    if (
      typeof timePreference !== "string" ||
      !["morning", "afternoon", "flexible"].includes(timePreference)
    ) {
      res.status(400).json({ error: "Invalid timePreference" });
      return;
    }
      ...
      .values({
        ...
        preferredDate:
          typeof preferredDate === "string" && preferredDate
            ...
            : null,
        flexibleDate:
          flexibleDate === true || flexibleDate === "true" ? "true" : "false",
        timePreference,
        notes: typeof notes === "string" && notes.trim() ? notes.trim() : null,
        hairColor:
          typeof hairColor === "string" && hairColor.trim()
            ? hairColor.trim()
            : null,
        addons:
          typeof addons === "string" && addons.trim() ? addons.trim() : null,
        basePrice: typeof basePrice === "number" ? basePrice : null,
        totalEstimate: typeof totalEstimate === "number" ? totalEstimate : null,
        status: "pending",
        clientPushToken:
          typeof clientPushToken === "string" && clientPushToken.trim()
            ? clientPushToken.trim()
            : null,
        clientWebPushSubscription:
          typeof clientWebPushSubscription === "string" &&
          clientWebPushSubscription.trim()
            ? clientWebPushSubscription.trim()
            : null,
      })
      .returning();
    ...
    const dateLabel = row!.preferredDate
      ? new Date(row!.preferredDate + "T12:00:00").toLocaleDateString("en-US", {
          weekday: "short",
          month: "short",
          day: "numeric",
        })
    ...
    const estimateLabel = row!.totalEstimate
      ? ` · $${row!.totalEstimate}+`
      : "";
    sendPushToAll(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const withLabel = rows.map((r) => ({
        ...r,
        serviceLabel: SERVICES[r.service] ?? r.service,
      }));
      res.json(withLabel);
    } catch (err) {
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const dateLabel = row.preferredDate
        ? new Date(row.preferredDate + "T12:00:00").toLocaleDateString(
            "en-US",
        ...
        if (row.clientWebPushSubscription) {
          try {
            const sub = JSON.parse(
              row.clientWebPushSubscription,
            ) as webpush.PushSubscription;
            void webpush
              .sendNotification(sub, JSON.stringify({ title, body, url }))
              .catch(() => {});
          } catch {
